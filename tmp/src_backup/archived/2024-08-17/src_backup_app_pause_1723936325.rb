...
class Pause
  ...
  def pause_buttons
    {
      unpause: Button.new(:pause, 540, 460, 'Return to Garden', 200, 50, :opaque),
  ...
  def mute_buttons(args)
    {
      mute_sfx: create_mute_button(:mute_sfx, 60, args.state.startup.sound_manager.sfx_gain, 'Unmute Sound Effects',
                                   'Mute Sound Effects'),
      mute_music: create_mute_button(:mute_music, 160, args.state.startup.sound_manager.music_gain, 'Unmute Music',
                                     'Mute Music')
    }
  end
