...
class Button
  ...
  def display(args)
    ...
    args.outputs.sprites << construct_button_sprite(args)
  end
  ...
  def construct_button_sprite(args)
    ...
    p spritesheet
    w, h = args.gtk.calcstringbox(@text, -4, 'fonts/Tiny5.ttf')
    [spritesheet.get(0, @x, @y, 3, h.to_i), spritesheet.get(1, @x + 3, @y, w.to_i, h.to_i),
     spritesheet.get(2, @x + w.to_i, @y, 3, h.to_i)]
  end
